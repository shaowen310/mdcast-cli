...
- Preprocessing for knowledge-base / RAG ingestion (deterministic structure — see *Output specification* below)
...
- Returns a `(md_path, {page: [filename, ...]})` tuple for downstream tools
...
`converter.py` (`BRAND_AREA_FRAC`, `BRAND_DIM_FRAC`, `BRAND_MARGIN_FRAC`).
...
The markdown produced by `pptx2md` is a **machine-readable, splittable, traceable** knowledge-base corpus. This section defines its deterministic structure so that an Agent can parse, chunk, and extract fields into a knowledge base directly, with no manual polishing required.
...
| Flow diagram | Rendered as `assets/slide-XX.png` plus a swimlane text version | See "Diagram handling" above |

> **Masters, backgrounds, and brand images are auto-excluded** (see the "Heuristic" table in *Brand / master image exclusion* above): `assets/` and image references only contain real content images. The rules cover three categories of decorative image: full-bleed backgrounds, page-spanning copies of the same background, and corner-anchored small brand logos / header-footer marks. No further filtering is needed at KB ingestion time.
